...
function nValidate($input, $form, regexp) {
  ...
  if (input.tagName === 'INPUT' || input.tagName === 'TEXTAREA') {
    ...
    if (input.type === 'radio') {
      ...
      let valueMatch = !$input.val().length ? 'empty' : 'notMatch'
  ...
  $eles.forEach((ele) => {
    ...
    $form.find('[required]')
      .on('blur', (e) => {
        ...
        let errorMsg = nValidate($this, $form, options.regexp)
        if (errorMsg) {
          showErrorTips({
            ele: $this[0],
            msg: errorMsg
...
function showErrorTips(error) {
  if (error) {
    const $ele = $(error.ele); const msg = error.msg
    ...
    const cellParent = nFindCellParent(error.ele)
...
function hideErrorTips(ele) {
  const cellParent = nFindCellParent(ele)
